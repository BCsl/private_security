 ...
 */
public class MainLockAdapter extends RecyclerView.Adapter<MainLockAdapter.MainLockViewHolder> {
    ...
    public MainLockAdapter(Context mContext) {
        this.mContext = mContext;
        packageManager = mContext.getPackageManager();
        mLockInfoManager = new CommLockInfoManager(mContext);
        userAppNum = SpUtil.getInstance().getInt(AppConstants.LOCK_USER_APP_NUM);
...
//        mCopyLockInfos.addAll(lockInfos);
    ...
    public MainLockViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_lock_top_list, parent, false);
    ...
    public void changeUserLockStatus(CheckBox checkBoxAll) {
        for (int i = 0; i < mLockInfos.size(); i++) {
            ...
            if (checkBoxAll.isChecked() && !lockInfo.isSysApp()) {
                SpUtil.getInstance().putBoolean(AppConstants.LOCK_IS_SELECT_ALL_APP, true);
                ...
                SpUtil.getInstance().putBoolean(AppConstants.LOCK_IS_SELECT_ALL_APP, false);
                lockInfo.setLocked(false);
                mLockInfoManager.unlockCommApplication(lockInfo.getPackageName());
            }
            //notifyItemChanged(i); 用这个界面会有闪一下的感觉
        }
        notifyDataSetChanged();
    }
