...
import com.tools.security.applock.db.FaviterDao;
 ...
 */
public class LockAppLoadService extends IntentService {

    public static final String ACTION_START_LOAD_APP = "com.tools.security.service.action.LOADAPP";
    public static final String ACTION_LOAD_APP_SUCCESS = "action_lock_app_success";
    private PackageManager mPackageManager;
    private CommLockInfoManager mLockInfoManager;
    long time = 0;

    public LockAppLoadService() {
        super("LockAppLoadService");
    }
    ...
    protected void onHandleIntent(Intent handleIntent) {
        if (handleIntent.getAction().equals(ACTION_START_LOAD_APP)) {
            ...
            if (isInitDb) {
                ...
                // appList = DataUtil.clearRepeatResolveInfo(appList);  //去除重复数据,得到新的列表
                // Log.i("onHandleIntent", "appList.size() = " + appList.size() + "  dbList.size() = " + dbList.size());
    ...
    public void initFavoriteApps() {
        FaviterDao faviterDao = new FaviterDao();
        ...
        faviterDao.insertFaviterInfoList(faviterInfos);
    }
}
