...
public class MyParticleSystem {
    ...
    private static class ParticleTimerTask extends TimerTask {
        ...
        private final WeakReference<MyParticleSystem> mPs;

        public ParticleTimerTask(MyParticleSystem ps) {
            mPs = new WeakReference<MyParticleSystem>(ps);
        }

        @Override
        public void run() {
            if (mPs.get() != null) {
                MyParticleSystem ps = mPs.get();
    ...
    private MyParticleSystem(ViewGroup parentView, int maxParticles, long timeToLive) {
        mRandom = new Random();
     ...
     */
    public MyParticleSystem(ViewGroup parentView, int maxParticles, Drawable drawable, long timeToLive) {
     ...
     * @param timeToLive    The time to live for the particles
     */
    public MyParticleSystem(Activity a, int maxParticles, int drawableRedId, long timeToLive) {
     ...
     */
    public MyParticleSystem(Activity a, int maxParticles, int drawableRedId, long timeToLive, int parentViewId) {
     ...
     * @param timeToLive   The time to live for the particles
     */
    public MyParticleSystem(Activity a, int maxParticles, Drawable drawable, long timeToLive) {
     ...
     * @param parentViewId The view Id for the parent of the particle system
     */
    public MyParticleSystem(Activity a, int maxParticles, Drawable drawable, long timeToLive, int parentViewId) {
     ...
     * @param timeToLive   The time to live for the particles
     */
    public MyParticleSystem(Activity a, int maxParticles, Bitmap bitmap, long timeToLive) {
     ...
     * @param parentViewId The view Id for the parent of the particle system
     */
    public MyParticleSystem(Activity a, int maxParticles, Bitmap bitmap, long timeToLive, int parentViewId) {
     ...
     * @param timeToLive   The time to live for the particles
     */
    public MyParticleSystem(Activity a, int maxParticles, AnimationDrawable animation, long timeToLive) {
     ...
     * @param parentViewId The view Id for the parent of the particle system
     */
    public MyParticleSystem(Activity a, int maxParticles, AnimationDrawable animation, long timeToLive, int parentViewId) {
        this((ViewGroup) a.findViewById(parentViewId), maxParticles, timeToLive);
        // Create the particles
        for (int i = 0; i < mMaxParticles; i++) {
            mParticles.add(new AnimatedParticle(animation));
     ...
     *
     * @param modifier modifier to be added to the MyParticleSystem
     */
    public MyParticleSystem addModifier(ParticleModifier modifier) {
        mModifiers.add(modifier);
        return this;
    }

    public MyParticleSystem setSpeedRange(float speedMin, float speedMax) {
     ...
     * @return This.
     */
    public MyParticleSystem setSpeedModuleAndAngleRange(float speedMin, float speedMax, int minAngle, int maxAngle) {
     ...
     * @return This.
     */
    public MyParticleSystem setSpeedByComponentsRange(float speedMinX, float speedMaxX, float speedMinY, float speedMaxY) {
     ...
     * @return This.
     */
    public MyParticleSystem setInitialRotationRange(int minAngle, int maxAngle) {
     ...
     * @return This.
     */
    public MyParticleSystem setScaleRange(float minScale, float maxScale) {
     ...
     * @return This.
     */
    public MyParticleSystem setRotationSpeed(float rotationSpeed) {
     ...
     * @return This.
     */
    public MyParticleSystem setRotationSpeedRange(float minRotationSpeed, float maxRotationSpeed) {
     ...
     * @return
     */
    public MyParticleSystem setAccelerationModuleAndAndAngleRange(float minAcceleration, float maxAcceleration, int minAngle, int maxAngle) {
     ...
     * @return This.
     */
    public MyParticleSystem setAcceleration(float acceleration, int angle) {
     ...
     * @return This.
     */
    public MyParticleSystem setParentViewGroup(ViewGroup viewGroup) {
    ...
    public MyParticleSystem setStartTime(long time) {
        mCurrentTime = time;
        return this;
    }
     ...
     */
    public MyParticleSystem setFadeOut(long milisecondsBeforeEnd, Interpolator interpolator) {
        mModifiers.add(new AlphaModifier(200, 0, mTimeToLive - milisecondsBeforeEnd, mTimeToLive, interpolator));
        return this;
    }
     ...
     */
    public MyParticleSystem setFadeOut(long duration) {
        return setFadeOut(duration, new LinearInterpolator());
    }

    public MyParticleSystem setFadeInOut(long duration) {
        Logger.i("duration=" + duration + ",mTimeToLive=" + mTimeToLive);
        if (duration > mTimeToLive) duration = mTimeToLive / 2;
        mModifiers.add(new AlphaModifier(0, 200, 0, duration, new LinearInterpolator()));
        mModifiers.add(new AlphaModifier(200, 0, duration, mTimeToLive, new LinearInterpolator()));
        return this;
    }

    public MyParticleSystem setFadeIn(long duration) {
        mModifiers.add(new AlphaModifier(0, 200, 0, duration, new LinearInterpolator()));
        return this;
    }
    ...
    public void emitWithGravity(View emiter, int gravity, float particlesPerSecond, int emitingTime) {
        // Setup emiter
        configureEmiter(emiter, gravity);
        startEmiting(particlesPerSecond, emitingTime);
    }
    ...
    public void emitWithGravity(View emiter, int gravity, float particlesPerSecond) {
        // Setup emiter
        configureEmiter(emiter, gravity);
        startEmiting(particlesPerSecond);
    }
    ...
    private void startEmiting(float particlesPerSecond) {
        mActivatedParticles = 0;
        mParticlesPerMilisecond = particlesPerSecond / 1000f;
        // Add a full size view to the parent view
        mDrawingView = new ParticleField(mParentView.getContext());
    ...
    private void startEmiting(float particlesPerSecond, int emitingTime) {
        mActivatedParticles = 0;
        mParticlesPerMilisecond = particlesPerSecond / 1000f;
        // Add a full size view to the parent view
        mDrawingView = new ParticleField(mParentView.getContext());
    ...
    public void emit(int emitterX, int emitterY, float particlesPerSecond) {
        configureEmiter(emitterX, emitterY);
        startEmiting(particlesPerSecond);
    }
    ...
    private void updateParticlesBeforeStartTime(float particlesPerSecond) {
        if (particlesPerSecond == 0) {
            return;
        }
        long currentTimeInMs = mCurrentTime / 1000;
        long framesCount = (long) (currentTimeInMs / particlesPerSecond);
        if (framesCount == 0) {
            return;
        }
        long frameTimeInMs = mCurrentTime / framesCount;
