...
public class SchedulerManager {
		...
		mBroadcastReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (ACTION_SCHEDULER_MANAGER.equals(action)) {
					...
					UtilTool.logStatic("taskSize:" + mHashMap.size());
					if (!TextUtils.isEmpty(taskKey) && mHashMap != null) {
						...
						if (task == null || task.isStop()) {
							UtilTool.logStatic("task is null");
							return;
						}
						task.execute();
						...
						if (intervalTime > 0) {
							...
							UtilTool.logStatic("intervaltime < 0");
							synchronized (mHashMap) {
						...
						UtilTool.logStatic("taskKey or mHashmap is null");
					}
				}
			}
		};
	...
	public void executeTask(SchedulerTask task) {
		if (mContext == null || mAlarmManager == null || task == null
				|| mHashMap == null) {
			UtilTool.logStatic("task null out");
			return;
		}
		synchronized (mHashMap) {
